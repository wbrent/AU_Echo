...
#endif
{
	...
	delTime = new float[getTotalNumInputChannels()];
...
void EchoDelayAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	delayBufferArray = new float*[getTotalNumInputChannels()];
...
void EchoDelayAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
		...
		for (int j = 0; j < blockSize; j++)
			channelData[j] += delayBufferArray[channel][delBufSizeSamps - (int)roundf(delTime[channel] * getSampleRate()) + j] * delFdbk;
